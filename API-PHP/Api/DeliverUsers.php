    ...
    public function getOpenid() {
        ...
        if (property_exists($rs,'openid')) {
            ...
            if ($res == -1) {
                ...
                switch ($res) {
                    case '1':
                        $to = "location:http://192.168.0.4:8080/?firstlogin=1&openid=".$rs->openid;
                        header($to);
                        break;
                    ...
                    default:
                        $to = "location:http://192.168.0.4:8080/?firstlogin=0&openid=".$rs->openid;
                        header($to);
                        break;
                }
            }
        ...
        if (property_exists($rs,'openid') && property_exists($rs,'access_token')) {
            ...
            if (property_exists($rrs,'nickname')) {
                ...
                if ($res == -1) {
                    ...
                    //跳转回去前端处理
                    $to = "location:https://192.168.0.4:8080/register?isregister=1&openid=".$openid;
                    header($to);
                }
            }else {
